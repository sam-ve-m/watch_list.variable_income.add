...
dummy_watch_list_symbols_model = [WatchListSymbolModel(symbol, 'test_id') for symbol in WatchListSymbols(**dummy_symbols_to_insert).symbols]
dummy_insert = [str(Sindri.dict_to_primitive_types(x.to_dict())) for x in dummy_watch_list_symbols_model]
...
@patch.object(WatchListRepository, "_WatchListRepository__get_collection")
async def test_insert_one_symbol_in_watch_list_exception(get_collection_mock, etria_mock):
    get_collection_mock.insert_one.side_effect = Exception("Erro")
    with pytest.raises(Exception):
        await WatchListRepository.insert_one_symbol_in_watch_list(dummy_watch_list_symbols_model[0])
        get_collection_mock.assert_called_once_with()
...
@patch.object(WatchListRepository, "_WatchListRepository__get_collection")
async def test_exists(get_collection_mock, symbol_finded):
    ...
    result = await WatchListRepository.exists(dummy_watch_list_symbols_model[0])
    get_collection_mock.assert_called_once_with()
...
async def test_exists_exception(get_collection_mock, etria_mock):
    ...
    with pytest.raises(Exception):
        await WatchListRepository.exists(dummy_watch_list_symbols_model[0])
        get_collection_mock.assert_called_once_with()
        etria_mock.assert_called()
